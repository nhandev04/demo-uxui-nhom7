...
export function Products() {
    ...
    if (selectedFlowerTypes.length > 0) {
        products = products.filter((p) => selectedFlowerTypes.includes(p.flowerType));
    }
    ...
    // Filter options
    const flowerTypes = ["Hồng", "Tulip", "Hướng dương", "Ly", "Cẩm chướng", "Lan", "Hoa baby", "Hoa mix"];
    const colors = [
        ...
        { name: "Xanh", value: "Xanh", colorClass: "#3b82f6" },
        {
            name: "Mix màu",
            value: "Mix màu",
            colorClass: "linear-gradient(135deg, #ec4899 0%, #f97316 50%, #fbbf24 100%)",
        },
    ];
    const occasions = [
        ...
        <Layout>
            <div className="bg-gradient-to-br from-[#fdf6f5] to-white py-8 sm:py-12">
                <div className="max-w-7xl mx-auto px-4 sm:px-6">
                    <h1 className="font-['Playfair_Display'] text-3xl sm:text-4xl text-[#6b1f15] mb-4">Sản phẩm</h1>
                    <p className="text-sm sm:text-base text-[#757575]">Khám phá bộ sưu tập hoa tươi cao cấp</p>
                </div>
            </div>
                                ...
                                <div className="space-y-3">
                                    {categories.map((cat) => (
                                        <label key={cat.id} className="flex items-center gap-3 cursor-pointer">
                                            ...
                                            />
                                            <span className="text-[#757575]">{cat.name}</span>
                                            <span className="text-xs text-[#9e9e9e] ml-auto">({cat.count})</span>
                                        </label>
                                    ))}
                                </div>
                            </div>
                                    ...
                                    {flowerTypes.map((type) => (
                                        <label key={type} className="flex items-center gap-2 cursor-pointer">
                                            <input
                                                ...
                                                onChange={(e) => {
                                                    if (e.target.checked) {
                                                        setSelectedFlowerTypes([...selectedFlowerTypes, type]);
                                                    } else {
                                                        setSelectedFlowerTypes(
                                                            selectedFlowerTypes.filter((t) => t !== type),
                                                        );
                                                    }
                                                }}
                                                className="w-4 h-4 text-[#e0997d] border-[#f0ccc3] rounded focus:ring-2 focus:ring-[#e0997d]/20"
                                            ...
                                            <input
                                                ...
                                                onChange={(e) => {
                                                    if (e.target.checked) {
                                                        ...
                                                        setSelectedOccasions(
                                                            selectedOccasions.filter((o) => o !== occasion.id),
                                                        );
                                                    }
                                                }}
                                                className="w-4 h-4 text-[#e0997d] border-[#f0ccc3] rounded focus:ring-2 focus:ring-[#e0997d]/20"
                                            />
                                            <span className="text-[#757575]">{occasion.name}</span>
                                        </label>
                                    ))}
                                </div>
                            </div>
                                    ...
                                    {[5, 4, 3].map((rating) => (
                                        <label key={rating} className="flex items-center gap-2 cursor-pointer">
                                            ...
                                            <div className="flex items-center gap-1">
                                                ...
                                                <span className="text-sm text-[#757575] ml-1">trở lên</span>
                                            </div>
                                        </label>
                                    ))}
                                </div>
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 sm:mb-8 bg-white rounded-xl border border-[#f5f5f5] p-4">
                            <div className="flex items-center justify-between sm:block">
                                <p className="text-sm sm:text-base text-[#757575]">
                                    Hiển thị <span className="text-[#424242]">{products.length}</span> sản phẩm
                                </p>
                                    ...
                                    {priceRange[1] < 5000000 && (
                                        <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-[#fdf6f5] border border-[#f0ccc3] rounded-lg text-sm">
                                            ...
                                            <span className="text-[#424242]">
                                                Giá: 0₫ - {formatPrice(priceRange[1])}₫
                                            </span>
                                            <button
                                    ...
                                    {selectedOccasions.map((occasionId) => (
                                        ...
                                        >
                                            ...
                                            <button
                                                onClick={() =>
                                                    setSelectedOccasions(
                                                        selectedOccasions.filter((o) => o !== occasionId),
                                                    )
                                                }
                                                className="hover:bg-[#f0ccc3] rounded-full p-0.5 transition-colors"
                                            >
                ...
                <div className="max-w-7xl mx-auto px-6">
                    <div className="bg-white rounded-2xl border border-[#f5f5f5] p-12 shadow-lg text-center">
                        ...
                        <p className="text-[#757575] text-lg mb-8 max-w-2xl mx-auto">
                            Đặt hoa theo yêu cầu riêng của bạn! Đội ngũ florist chuyên nghiệp sẽ tạo nên những bó hoa
                            độc đáo, phù hợp hoàn hảo với phong cách và dịp của bạn.
                        </p>
                                    ...
                                    {categories.map((cat) => (
                                        <label key={cat.id} className="flex items-center gap-3 cursor-pointer">
                                            <input
                                                ...
                                                className="w-4 h-4 text-[#e0997d] border-[#f0ccc3] rounded focus:ring-2 focus:ring-[#e0997d]/20"
                                            />
                                            <span className="text-[#757575] text-sm">{cat.name}</span>
                                            <span className="text-xs text-[#9e9e9e] ml-auto">({cat.count})</span>
                                        </label>
                                    ))}
                                </div>
                                    ...
                                    {flowerTypes.map((type) => (
                                        <label key={type} className="flex items-center gap-2 cursor-pointer">
                                            <input
                                                ...
                                                className="w-4 h-4 text-[#e0997d] border-[#f0ccc3] rounded focus:ring-2 focus:ring-[#e0997d]/20"
                                            />
                                            <span className="text-[#757575] text-sm">{type}</span>
                                        </label>
                                    ))}
                                </div>
                            </div>
                                    ...
                                    {occasions.map((occasion) => (
                                        <label key={occasion.id} className="flex items-center gap-2 cursor-pointer">
                                            <input
                                                ...
                                                className="w-4 h-4 text-[#e0997d] border-[#f0ccc3] rounded focus:ring-2 focus:ring-[#e0997d]/20"
                                            />
                                            <span className="text-[#757575] text-sm">{occasion.name}</span>
                                        </label>
                                    ))}
                                </div>
                            </div>
...
function ProductCardGrid({ product }: { product: any }) {
    return (
        <div className="group bg-white rounded-2xl overflow-hidden border border-[#f5f5f5] hover:border-[#f0ccc3] hover:shadow-xl transition-all duration-300">
            <div className="relative h-64 overflow-hidden bg-[#fafafa]">
                ...
                {product.originalPrice && (
                    <div className="absolute top-4 left-4">
                        <span className="px-3 py-1 bg-[#ef4444] text-white text-sm rounded-full">Sale</span>
                    </div>
                )}
            ...
            <div className="p-5">
                <Link to={`/products/${product.id}`}>
                    <h3 className="text-[#424242] mb-2 group-hover:text-[#e0997d] transition-colors">{product.name}</h3>
                </Link>
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <span className="text-xl text-[#e0997d]">{formatPrice(product.price)}₫</span>
                        {product.originalPrice && (
        ...
        <div className="group bg-white rounded-2xl overflow-hidden border border-[#f5f5f5] hover:border-[#f0ccc3] hover:shadow-xl transition-all duration-300">
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 p-4 sm:p-6">
                ...
                <div className="flex-1 flex flex-col min-w-0">
                    <div className="flex items-start justify-between gap-3 mb-3">
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2">
                                <div className="flex items-center gap-1">
                                    <Star className="w-4 h-4 fill-[#f59e0b] text-[#f59e0b]" />
                                    <span className="text-xs sm:text-sm text-[#757575]">{product.rating}</span>
                                </div>
                                <span className="text-xs text-[#9e9e9e]">({product.reviews} đánh giá)</span>
                            </div>
                        </div>
                    ...
                    <p className="text-sm sm:text-base text-[#9e9e9e] mb-4 line-clamp-2">{product.description}</p>
